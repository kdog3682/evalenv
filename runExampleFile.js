...
import { ScopedEvaluator } from "./ScopedEvaluator.js"
...
const baseBindings = {
    ...
    lines({ s, lines, arg, text }) {
        if (lines) {
            return JSON.stringify(lines)
        }
        const computed = s || arg || text
        return JSON.stringify(lineGetter(computed))
    },
}
...
const lezerBindings = {
    state: "await lezer.createState(text)",
}

function isVimState(x) {
    return isObject(x) && x.file && (x.lnum || x.lineNumber)
}

async function importation(file) {
    return await import(file)
}

function addImplicitField(s, field) {
    const r = RegExp("^" + field + ":", "mi")
    if (r.test(s)) {
        return s
    }
    // s = s.trim()
    let touched = false
    let value = replaceLast(s, /^\w+:[^]+?(?=\n\n)/m, (x) => {
        touched = true
        return x + "\n\n" + field + ":\n"
    })
    if (touched) {
...
async function dashed(text, options = {}) {
    assignFresh(options, {
        implicitField: "code",
        removeStartingComments: true,
    })

    if (options.removeStartingComments) {
        text = text.replace(/^\/\/.+/gm, "")
    }
    if (options.implicitField) {
        text = addImplicitField(text, options.implicitField)
    }
    ...
    if (options.hooks) {
        Object.assign(ref.hooks, options.hooks)
    }

    ref.bindings = options.bindings || {}
    ref.vim = options.vim || {}
    const config = colonConfig(text, ref)

    const get = async (config) => {
        if (config.exit) {
            Deno.exit()
        } else if (config.use) {
            const module = await import(config.use)
            const fn = must(module, "default")
            return fn(getArg(config))
        } else {
            const modules = await loadModules(
                config.code,
                options,
            )
            const evaluator = ScopedEvaluator.from(modules)
            return await evaluator(config.code)
        }
    }
    const result = await get(config)
    return await finish(result, config, options)
}
...
const modules = await Promise.all(files.map(importation))

const dynamicImports = [
    {
        name: "vim",
        file: "/home/kdog3682/2024-javascript/stdlib/vim/index.js",
        regex: /\bvim\./,
    },
    {
        name: "foobar",
        file: "/home/kdog3682/2024-javascript/stdlib/foobar/index.js",
        regex: /\bfoobar\./,
    },
    {
        name: "lezer",
        file: "/home/kdog3682/2024-javascript/stdlib/lezer/index.js",
        regex: /main\(state\)/,
        modules: "/home/kdog3682/2024-javascript/stdlib/lezer/functions.js",
    },
]
...
const exampleFileOptions = {
    finisherDir: '/home/kdog3682/2024-javascript/evalenv/finishers',
...
function getArg(config) {
    if (config.inpath) {
        return read(config.inpath)
    }
    return config.arg || config.s || config.text
}
async function loadModules(code, options) {
    const modules = toArray(options.modules)

    for (const c of toArray2(options.dynamicImports)) {
        if (c.regex.test(code)) {
            const module = await import(c.file)
            if (c.name) {
                const p = { [c.name]: module.default || module }
                modules.push(p)
            } else {
                modules.push(module)
            }

            for (const file of toArray2(c.modules)) {
...
async function runExampleFile(vim) {
    const text = getVimSectionText(vim)
    return await dashed(text, exampleFileOptions)
}

...
function createCodeString(s, config, options) {
    ...
    if (config.fn) {
        const callable = strcall(config.fn, s)
        const callString = tryString(callable, onError)
        return callString
    }

    if (hasStartingCallable(s)) {
        return s
    }

    const lib = codeLibrary(s)
    const { name, params } = getMain(lib)
    // throw config

    const bindings = buildBindings(params, options.bindings, config)
    const filteredParams = shared(params, Object.keys(bindings))
    const callable = strcall(name, filteredParams)
    const bindingString = join(map(bindings, simpleBinding))
    if (/\bawait\b/.test(bindingString)) {
        const callString = 'return ' + callable
        const preCode = join(s, bindingString, callString)
        return asyncWrapper(preCode, onError)
    } else {
        const preCode = join(s, bindingString, callable)
        return tryString(preCode, onError)
        
    }
}
function asyncWrapper(code, onError) {
    const s = tryString(code, onError, 'asynchronous')
    const t = `async function __main__() {\n\t$1\n}\n__main__()`
    return templater(t, [s])
}

function buildBindings(bindings, ref, config) {
    const store = {}
    const regex = reWrap(ref, /(?:^|[^'"\w])($1)(?:[^'"\w]|$)/g)
    const a = regex.test("text")
    run(bindings)
    return store

    function run(bindings) {
        const clean = filter4(bindings, store, true)
        for (const binding of clean) {
            assign(store, binding, runner(binding))
        }
    }
    ...
    function runner(binding) {
        ...
        } else if (isFunction(paramString)) {
            return paramString(config)
        } else {
            const matches = findall(regex, paramString)
            run(matches)
            return paramString
        }
    }
...
function getDefaultArgString(config) {
    if (config.inpath) {
        return `read("${config.inpath}")`
    }
    if (config.hasOwnProperty("arg")) {
        return getBindingValueString(config.arg)
    }
    if (config.hasOwnProperty("s")) {
        return getBindingValueString(config.s)
    }
    if (config.hasOwnProperty("text")) {
        return getBindingValueString(config.text)
    }
}
function getFromSource(binding, config) {
    if (isString(binding)) {
        const base = ["inpath", "arg", "s", "text"]
        if (base.includes(binding)) {
            return getDefaultArgString(config)
        }
...
const lezerBindings = {
    tree: "getTree(text, lang)",
    grammar: "source",
    text: "source",
    s: "source",
    arg: "source",
    inpath: "source",
    lang({ inpath, text, lang }) {
        if (lang) {
            return doubleQuote(lang)
        }
        if (inpath) {
            return doubleQuote(inferLang(inpath))
        }
        if (text) {
            return doubleQuote(inferLang(text))
        }
        return "null"
    },
    node: "tree.topNode",
    getText: "getStatef(text, 'text')",
    getState: "getStatef(text)",
    state: "{node, getText, tree}",
    lezerState: "{node, getText, tree, grammar}"
}

// console.log(buildBindings(['lezerState', 'lezerState', 'text'], lezerBindings, {'text': 'function foo() {}'}))
